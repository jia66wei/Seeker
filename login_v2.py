...
class Login:
	...
	def loginWithCookies(self, cookie_uid):
	...
	def loginSimulation(self,username,password):
		...
		CAPTCHA = raw_input(u"请输入验证码:").decode('utf8').encode('gbk')
...
if __name__ == '__main__':
	if len(sys.argv) < 2:
		print "need 'exe','userfile'"
	login = Login()
	user_file = sys.argv[1]
	fin = open(user_file,'r')
	for line in fin:
		if line.startswith('#'):continue
		user_name,passwd,uid = line.rstrip('\n').split('/')
		print user_name,passwd,uid
		try:
			login.loginSimulation(user_name,passwd)
			time.sleep(5)
		except Exception,e:
			print e
